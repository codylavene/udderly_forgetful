...
export class List {
    ...
    async addList(body) {
        try {
            ...
            if (!data.message) {
                ...
                document.querySelector(".added-list-child-container").appendChild(div);
                document.getElementById("delete-list-input").appendChild(option);
                document.querySelector(".list-drop-down").appendChild(optionForEdit);
                document.getElementById("add-list-input").value = "";
                document.querySelector(".add-list-container").classList.toggle("hide-list-container");
            }
        } catch (e) {
            console.error(e);
        }
    ...
    async renderList(listId, e) {
        ...
        document.querySelector(".current-list-stats").innerHTML = `Stats for ${e.target.innerHTML} list`;
    ...
    async deleteList() {
        ...
        if (data.message === "Destroyed") {
            ...
            for (let i = 0; i < children.length; i++) {
                ...
                if (child.id === id) {
                    document.querySelector(".added-list-child-container").removeChild(child);
                }
            }
